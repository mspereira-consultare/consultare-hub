...
export async function GET(_: Request, context: ParamsContext) {
  try {
    ...
    worksheet.getCell('A2').value = project.description || 'Cronograma exportado do módulo de tarefas da intranet.';
    ...
    return new NextResponse(output, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="projeto-${project.name.toLowerCase().replace(/\s+/g, '-')}.xlsx"`,
      },
    });
  } catch (error: any) {
    console.error('Erro ao exportar projeto de tarefas da intranet em XLSX:', error);
