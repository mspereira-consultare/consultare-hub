...
import { addTaskAttachment } from '@consultare/core/tasks/repository';
...
const buildStorageKey = (taskId: string, originalName: string) => {
  ...
  return `${prefix}/${taskId}/attachments/${stamp}-${fileName}`;
};
...
export async function POST(request: Request, context: ParamsContext) {
  try {
    ...
    if (!cleanTaskId) return NextResponse.json({ error: 'taskId inválido.' }, { status: 400 });
    ...
    const key = buildStorageKey(cleanTaskId, originalName);
    ...
    try {
      ...
      if (uploaded?.key) {
        try {
          ...
          console.error('Falha ao limpar anexo da tarefa após erro:', cleanupError);
        }
      }
      throw error;
    }
  } catch (error: any) {
    console.error('Erro ao anexar arquivo à tarefa:', error);
    return NextResponse.json({ error: error?.message || 'Erro interno no upload do anexo.' }, { status: Number(error?.status) || 500 });
  }
}
