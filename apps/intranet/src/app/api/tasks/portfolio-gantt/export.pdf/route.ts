...
export async function GET() {
  try {
    ...
    page.drawText('Portfólio consolidado de tarefas', { x: 42, y: 550, size: 18, font: fontBold, color: rgb(1, 1, 1) });
    page.drawText('Resumo dos projetos visíveis e das tarefas avulsas agendadas.', {
      x: 42,
      y: 525,
      size: 9,
      font,
    ...
    let cursorY = 490;
    for (const section of scheduledSections.slice(0, 8)) {
    ...
    return new NextResponse(Buffer.from(output), {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="portfolio-consolidado-intranet-${new Date().toISOString().slice(0, 10)}.pdf"`,
      },
    });
  } catch (error: any) {
    console.error('Erro ao exportar portfólio gantt da intranet em PDF:', error);
