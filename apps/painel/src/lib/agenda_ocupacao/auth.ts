...
export const requireAgendaOcupacaoPermission = async (action: PermissionAction) => {
  ...
  const allowed = hasPermission(permissions, 'agenda_ocupacao', action, role);

  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para agenda de ocupacao.' };
  }
