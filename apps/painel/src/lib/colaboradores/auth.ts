...
export const requireColaboradoresPermission = async (action: PermissionAction) => {
  ...
  const allowed = hasPermission(permissions, 'colaboradores', action, role);

  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para colaboradores.' };
  }
