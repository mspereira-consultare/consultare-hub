...
export const requireDashboardPermission = async (action: PermissionAction) => {
  ...
  const allowed = hasPermission(permissions, 'dashboard', action, role);

  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para o painel executivo.' };
  }
