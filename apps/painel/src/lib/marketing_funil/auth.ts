...
export const requireMarketingFunilPermission = async (action: PermissionAction) => {
  ...
  const allowed = hasPermission(permissions, 'marketing_funil', action, role);

  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para marketing funil.' };
  }
