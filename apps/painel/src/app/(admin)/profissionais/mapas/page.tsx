  ...
  return (
    <div className="space-y-0.5">
      {values.map((value, index) => (
        <div key={`${value}-${index}`} className="leading-4 text-slate-700">
          {value}
        </div>
      ))}
    </div>
...
export default function ProfissionaisMapasPage() {
  ...
  return (
    <div className="p-8 max-w-[1880px] mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 2xl:grid-cols-2">
                {specialty.units.map((unit) => (
                  <div key={`${specialty.specialty}-${unit.serviceUnit}`} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="bg-[#17407E] px-4 py-2.5 text-center text-sm font-semibold uppercase tracking-[0.08em] text-white">
                      ...
                      <table className="w-full table-fixed border-collapse text-[13px]">
                        ...
                        <thead>
                          <tr className="bg-slate-100 text-xs font-semibold uppercase tracking-wide text-slate-600">
                            <th className="border-b border-r border-slate-200 px-4 py-2 text-left">Dia</th>
                            <th className="border-b border-r border-slate-200 px-4 py-2 text-left">M</th>
                            <th className="border-b border-r border-slate-200 px-3 py-2 text-left">Freq.</th>
                            <th className="border-b border-r border-slate-200 px-4 py-2 text-left">T</th>
                            <th className="border-b border-slate-200 px-3 py-2 text-left">Freq.</th>
                          </tr>
                        </thead>
                        <tbody>
                          {unit.rows.map((row) => (
                            <tr key={`${unit.serviceUnit}-${row.weekday}`} className="align-top">
                              <td className="border-b border-r border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-600">
                                {WEEKDAY_LABELS[row.weekday] || row.weekday}
                              </td>
                              <td className="border-b border-r border-slate-200 px-4 py-2 align-top">
                                {renderCellLines(
                                  row.morning.map((entry) => formatProfessionalDisplayName(entry.professionalName))
                                )}
                              </td>
                              <td className="border-b border-r border-slate-200 px-3 py-2 align-top text-[12px] text-slate-600">
                                {renderCellLines(
                              ...
                              <td className="border-b border-slate-200 px-3 py-2 align-top text-[12px] text-slate-600">
                                {renderCellLines(
                                  row.afternoon.map(
