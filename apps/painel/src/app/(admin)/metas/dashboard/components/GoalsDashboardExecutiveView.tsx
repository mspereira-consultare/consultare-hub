...
function StatusList({
  ...
  return (
    <div className={`rounded-xl border ${palette.wrapper} p-4 shadow-sm`}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="rounded-full bg-white/80 p-1.5 text-slate-700 shadow-sm">{icon}</div>
          <div>
            <h3 className={`text-[13px] font-semibold ${palette.title}`}>{title}</h3>
            <p className="text-[11px] text-slate-500">Ordenado por nome para facilitar a leitura.</p>
          </div>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${palette.chip}`}>{goals.length}</span>
      </div>
        ...
        <div className="max-h-[360px] space-y-2.5 overflow-y-auto pr-1">
          {goals.map((goal) => (
            <button
              ...
              className="flex w-full items-start justify-between gap-4 rounded-lg border border-white/80 bg-white/90 px-4 py-3 text-left transition hover:border-slate-200 hover:shadow-sm"
            >
              <div className="min-w-0">
                <div className="text-sm font-semibold text-slate-800">{goal.name}</div>
                <div className="mt-1 text-[11px] text-slate-500">{goal.scope === 'CARD' ? 'Resolve' : goal.sector || 'Clínica'}</div>
              </div>
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[13px] font-semibold text-slate-800">Resumo por área</h3>
            <p className="text-[11px] text-slate-500">Panorama consolidado para identificar onde estão os maiores gargalos.</p>
          </div>
        </div>
        ...
        {sectorSummaries.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
            {sectorSummaries.map((summary) => {
              ...
              return (
                <div key={summary.sector} className="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-slate-800">{summary.sector}</div>
                      <div className="mt-1 text-[11px] text-slate-500">{summary.totalGoals} meta(s) ativas no recorte</div>
                    </div>
                    <div className="rounded-full bg-white px-3 py-1 text-xs font-bold text-slate-700 shadow-sm">{summary.avgPercentage}%</div>
                  </div>
