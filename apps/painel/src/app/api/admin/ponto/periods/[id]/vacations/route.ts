...
import { listPayrollVacationRows } from '@/lib/payroll/repository';
...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    const data = await listPayrollVacationRows(auth.db, String(id || ''), parsePayrollLineFilters(searchParams));
    return NextResponse.json({ status: 'success', data });
  } catch (error: any) {
    console.error('Erro ao listar férias sincronizadas:', error);
