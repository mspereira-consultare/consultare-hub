...
import {
  listMarketingFunnelCampaignLandingPages,
...
const getFilters = (request: Request): MarketingFunilFilters => {
  ...
  return {
    ...
    landingPage: searchParams.get('landingPage') || undefined,
  };
};
...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    const data = await listMarketingFunnelCampaignLandingPages(auth.db, String(campaignKey || ''), getFilters(request));
    return NextResponse.json({ status: 'success', data });
  } catch (error: unknown) {
    const status = error instanceof MarketingFunilValidationError ? error.status : 500;
    console.error('Erro API marketing/funil campaign landing-pages:', error);
