...
export async function POST(_: Request, context: ParamsContext) {
  try {
    ...
    const updated = await activateContractTemplate(auth.db, String(id || ''), auth.userId);
    return NextResponse.json({ status: 'success', data: updated });
  } catch (error: any) {
    console.error('Erro ao ativar modelo de contrato:', error);
    const status =
      error instanceof ContractTemplateValidationError
        ? error.status
        : Number(error?.status) || 500;
    return NextResponse.json(
      { error: error?.message || 'Erro interno ao ativar modelo.' },
      { status }
    );
  }
}
