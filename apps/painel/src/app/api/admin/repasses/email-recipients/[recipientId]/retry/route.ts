...
import {
  createRetryRepasseEmailRecipientJob,
...
export async function POST(_request: Request, context: ParamsContext) {
  try {
    ...
    const data = await createRetryRepasseEmailRecipientJob(auth.db, recipientId, auth.userId);
    return NextResponse.json({ status: 'success', data });
  } catch (error: unknown) {
    console.error('Erro ao reenviar destinatario de e-mail de repasse:', error);
    const status =
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Erro interno ao reenviar destinatario de e-mail de repasse.',
      },
      { status }
    );
  }
