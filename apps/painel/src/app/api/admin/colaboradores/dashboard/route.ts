...
export async function GET(request: Request) {
  try {
    ...
    const data = await getEmployeeDashboard(auth.db, {
    ...
    console.error('Erro ao carregar dashboard de colaboradores:', error);
    const status = Number(error?.status) || 500;
    return NextResponse.json({ error: error?.message || 'Erro interno ao carregar dashboard.' }, { status });
  }
}
