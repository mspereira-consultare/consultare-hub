...
export async function POST(request: Request, context: ParamsContext) {
  try {
    ...
    const data = await rejectPortalPersonalData(
      auth.db,
      String(submissionId || ''),
      auth.userId,
      String(body?.reason || '')
    ...
    console.error('Erro ao rejeitar dados pessoais do portal:', error);
    return NextResponse.json(
      { error: getEmployeePortalErrorMessage(error, 'Erro interno ao rejeitar dados pessoais.') },
