...
import { listPayrollHoursBalanceRows } from '@/lib/payroll/repository';
...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    const data = await listPayrollHoursBalanceRows(auth.db, String(id || ''), parsePayrollLineFilters(searchParams));
    return NextResponse.json({ status: 'success', data });
  } catch (error: any) {
    console.error('Erro ao listar banco de horas da folha:', error);
