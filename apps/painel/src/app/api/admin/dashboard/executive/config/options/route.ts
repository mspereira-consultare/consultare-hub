...
import { getExecutiveScopeOptions } from '@/lib/dashboard_executive/repository';
...
export async function GET() {
  try {
    ...
    const data = await getExecutiveScopeOptions(db);
    return NextResponse.json({ status: 'success', data });
  } catch (error: any) {
    console.error('Erro GET executive config options:', error);
