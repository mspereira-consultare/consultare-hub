...
import { getQmsDocumentFileById } from '@/lib/qms/repository';
...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    const file = await getQmsDocumentFileById(auth.db, String(id || ''), String(fileId || ''));
    if (!file) {
    ...
    console.error('Erro ao baixar arquivo QMS:', error);
