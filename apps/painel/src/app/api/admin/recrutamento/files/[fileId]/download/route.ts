...
import { getRecruitmentCandidateFileById } from '@/lib/recrutamento/repository';
...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    const file = await getRecruitmentCandidateFileById(auth.db, String(fileId || ''));
    ...
    console.error('Erro ao baixar arquivo de candidato:', error);
    return NextResponse.json({ error: details?.message || 'Erro interno ao baixar arquivo.' }, { status: Number(details?.status) || 500 });
  }
}
