...
DB=ROOT/'apps'/'painel'/'docs'/'database'
...
LEGACY=[
 'apps/painel/docs/03-arquitetura-tecnica.md','apps/painel/docs/04-dicionario-de-dados.md','apps/painel/docs/07-plano-tecnico-repasses.md','apps/painel/docs/08-agenda-ocupacao.md','apps/painel/docs/09-plano-tecnico-marketing-funil.md','apps/painel/docs/10-plano-tecnico-colaboradores.md','apps/painel/docs/11-plano-tecnico-equipamentos.md','apps/painel/docs/13-plano-tecnico-vigilancia-sanitaria.md','apps/painel/docs/14-plano-tecnico-folha-pagamento.md']
PURPOSE={
...
def domain(name,w):
    ...
    if name.startswith(('clinia_','agenda_','recepcao_','monitor_','espera_','crc_')):
        return 'mkt' if name.startswith(('fact_clinia_ads_','raw_clinia_ads_')) else 'ops'
    ...
    if w:
        ...
        if any(x in w for x in ['marketing','clinia_ads']):return 'mkt'
...
def search(names):
    ...
    for p in files():
        ...
        for n in ms:
            out[n]['all'].append(rp)
            if rp.startswith('apps/painel/docs/'):out[n]['docs'].append(rp)
...
def prio(p):
    ...
    if p.startswith('apps/painel/src/lib/'):return (2,len(p),p)
    if p.startswith('apps/painel/src/app/api/'):return (3,len(p),p)
    if p.startswith('apps/painel/scripts/'):return (4,len(p),p)
    if p.startswith('apps/painel/docs/'):return (6,len(p),p)
    return (9,len(p),p)
...
def writeproc(w):
    ...
    if w.startswith('apps/painel/src/lib/'):return f'Escrita principal realizada por repository/servico server-side em `{w}`.'
    if w.startswith('apps/painel/src/app/api/'):return f'Escrita principal garantida por rota API em `{w}`.'
    if w.startswith('apps/painel/scripts/'):return f'Escrita/garantia de schema em script `{w}`.'
