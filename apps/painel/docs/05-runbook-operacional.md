...
- `WORKER_HEALTHCHECK_ENABLED` (opcional, padrão `auto`; sobe `/healthz` e `/readyz` no worker)
- `WORKER_HEALTHCHECK_PORT` (opcional, padrão `PORT` do Railway ou `8080`)
- `PLAYWRIGHT_MAX_CONCURRENT_SESSIONS` (opcional, padrão `1`, recomendado manter baixo no Railway)
