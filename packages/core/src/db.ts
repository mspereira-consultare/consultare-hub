...
export function ensureServerEnv() {
  ...
  try {
    // Fallback local: quando o Next é iniciado dentro de um app em `apps/*`,
    // o `.env` na raiz do repositório não é carregado automaticamente.
    // Tentamos alguns caminhos para manter os apps consistentes no ambiente local.
...
function getMysqlDbConnection(): DbInterface {
  ...
  const createMysqlConnectionDb = (connection: PoolConnection): DbInterface => {
...
export const runInTransaction = async <T>(
  db: DbInterface,
  work: (transactionDb: DbInterface) => Promise<T>
) => {
