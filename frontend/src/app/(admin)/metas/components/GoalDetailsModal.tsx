...
import { X, Calendar, Target, TrendingUp, Activity, AlertCircle } from 'lucide-react';
import {
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
              ...
              <div className="text-2xl font-bold text-slate-800">{formatValue(targetValue)}</div>
            </div>
            ...
            <div className={`${bgClass} p-4 rounded-xl border border-transparent`}>
              ...
              <div className={`text-2xl font-bold ${colorClass}`}>{formatValue(currentData?.current || 0)}</div>
            </div>

            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                <div className="text-slate-500 text-xs font-bold uppercase">Atingimento</div>
                <div className={`text-3xl font-bold ${colorClass}`}>{Math.round(currentData?.percentage || 0)}%</div>
              </div>
          ...
          <div>
            ...
            <div className="h-64 w-full bg-slate-50 rounded-xl border border-slate-100 p-4 relative">
              {loading ? (
                <div className="absolute inset-0 flex items-center justify-center text-slate-400">Carregando histórico...</div>
              ) : history.length > 0 ? (
