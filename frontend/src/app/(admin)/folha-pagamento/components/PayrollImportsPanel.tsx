  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[360px,1fr]">
      <div className="space-y-4">
        ...
        />
        <UploadCard
          title="Planilha de referência (XLSX)"
          description="Usada para a comparação e auditoria do fechamento."
          buttonLabel="Enviar XLSX"
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-4 py-3">
          ...
          <p className="mt-1 text-xs text-slate-500">Os arquivos mais recentes substituem a base ativa da competência, mas o histórico fica registrado.</p>
        </div>
        <div className="max-h-[520px] overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {imports.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-16 text-center text-slate-500">Nenhum arquivo enviado nesta competência.</td></tr>
              ) : imports.map((item) => (
                <tr key={item.id} className="border-t border-slate-100 align-top">
                  <td className="px-4 py-3 font-medium text-slate-900">{item.fileName}</td>
                  <td className="px-3 py-3 text-slate-600">{item.fileType === 'POINT_PDF' ? 'Ponto (PDF)' : 'Referência (XLSX)'}</td>
                  <td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${item.processingStatus === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' : item.processingStatus === 'FAILED' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{item.processingStatus}</span></td>
...
function UploadCard({ title, description, buttonLabel, loading, accept, onPick, icon: Icon }: { title: string; description: string; buttonLabel: string; loading: boolean; accept: string; onPick: (file: File) => void; icon: typeof UploadCloud }) {
  return (
    <label className="block cursor-pointer rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:border-slate-300">
      ...
      <div className="flex items-start gap-3">
        <div className="rounded-xl bg-blue-50 p-3 text-[#17407E]"><Icon size={18} /></div>
