...
export function PayrollClosingTable({ rows, loading, onOpenDetail }: { rows: PayrollLine[]; loading: boolean; onOpenDetail: (line: PayrollLine) => void }) {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-4 py-3">
        <h3 className="text-sm font-semibold text-slate-800">Fechamento operacional</h3>
        <p className="mt-1 text-xs text-slate-500">Clique na linha para abrir memória de cálculo, ponto e ajustes manuais.</p>
      </div>
        ...
        <table className="min-w-[1380px] w-full text-sm">
          <thead className="sticky top-0 z-10 bg-slate-50 text-xs uppercase tracking-[0.18em] text-slate-500">
            <tr>
              ...
              <th className="px-3 py-3 text-left">Comparação</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={17} className="px-4 py-16 text-center text-slate-500">Carregando linhas da folha...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={17} className="px-4 py-16 text-center text-slate-500">Nenhuma linha gerada para a competência atual.</td></tr>
            ) : rows.map((line) => (
              <tr key={line.id} className="border-t border-slate-100 hover:bg-slate-50/70 cursor-pointer" onClick={() => onOpenDetail(line)}>
                <td className="sticky left-0 z-[1] bg-white px-4 py-3 group-hover:bg-slate-50/70">
                ...
                <td className="px-3 py-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">{statusLabelMap[line.lineStatus] || line.lineStatus}</span></td>
                <td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${line.comparisonStatus === 'DIVERGENTE' ? 'bg-rose-100 text-rose-700' : line.comparisonStatus === 'IGUAL' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{statusLabelMap[line.comparisonStatus] || line.comparisonStatus}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
