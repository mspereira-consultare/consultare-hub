  ...
  const components: Components = {
    ...
    h3({ className, ...props }) {
      return (
        <h3
          {...props}
          className={cn('mt-6 scroll-mt-24 text-lg font-semibold text-slate-900', className)}
        />
      );
    },
    p({ className, ...props }) {
      ...
      if (normalized.startsWith('#')) {
        return (
          <a
            ...
            href={normalized}
            className={cn(
              'text-consultare-navy underline decoration-consultare-teal/40 hover:decoration-consultare-teal',
              className
            )}
          >
            {children}
