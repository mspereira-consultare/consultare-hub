...
export async function GET(_: Request, context: ParamsContext) {
  try {
    ...
    const data = await listEmployeeUniformItems(auth.db, String(id || ''));
    return NextResponse.json({ status: 'success', data });
  } catch (error: any) {
    console.error('Erro ao listar uniformes:', error);
    return NextResponse.json({ error: error?.message || 'Erro interno ao listar uniformes.' }, { status: Number(error?.status) || 500 });
  }
}
...
export async function POST(request: Request, context: ParamsContext) {
  try {
    ...
    const data = await saveEmployeeUniformItem(auth.db, String(id || ''), body, auth.userId);
    return NextResponse.json({ status: 'success', data });
  } catch (error: any) {
    ...
    return NextResponse.json({ error: error?.message || 'Erro interno ao salvar uniforme.' }, { status });
  }
}
