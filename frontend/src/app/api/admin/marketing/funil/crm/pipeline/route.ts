...
export async function GET(request: Request) {
  try {
    ...
    const data = await listMarketingFunilCrmPipeline(auth.db, getFilters(request));
    return NextResponse.json({ status: 'success', data });
  } catch (error: unknown) {
    const status = error instanceof MarketingFunilValidationError ? error.status : 500;
    console.error('Erro API marketing/funil crm/pipeline:', error);
