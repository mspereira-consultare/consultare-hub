...
import { upsertRepasseConsolidacaoNote } from '@/lib/repasses/repository';
...
export async function PUT(request: Request) {
  try {
    ...
    const saved = await upsertRepasseConsolidacaoNote(
      auth.db,
      {
    ...
    console.error('Erro ao salvar observacao de repasse a consolidar:', error);
    return NextResponse.json(
      { error: error?.message || 'Erro interno ao salvar observacao de repasse a consolidar.' },
