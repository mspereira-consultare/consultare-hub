...
export async function GET(request: Request, context: ParamsContext) {
  try {
    ...
    console.error('Erro ao buscar detalhes de repasse a consolidar por profissional:', error);
    const status = Number(error?.status) || 500;
    return NextResponse.json(
      { error: error?.message || 'Erro interno ao buscar detalhes de repasse a consolidar.' },
      { status }
    );
  }
}
