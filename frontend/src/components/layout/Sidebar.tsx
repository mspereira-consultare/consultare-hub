...
const menuItems: MenuItem[] = [
  { href: "/dashboard", label: "Visao Geral", icon: LayoutDashboard, group: "PRINCIPAL", roles: ["ADMIN", "GESTOR", "OPERADOR"], pageKey: "dashboard" },
  { href: "/monitor", label: "Monitor de Atendimento", icon: PhoneCall, group: "OPERACOES", roles: ["ADMIN", "GESTOR", "OPERADOR"], pageKey: "monitor" },
  { href: "/checklist-crc", label: "Checklist CRC", icon: MessageCircle, group: "OPERACOES", roles: ["ADMIN", "GESTOR", "OPERADOR"], pageKey: "checklist_crc" },
  { href: "/checklist-recepcao", label: "Checklist Recepcao", icon: ClipboardList, group: "OPERACOES", roles: ["ADMIN", "GESTOR", "OPERADOR"], pageKey: "checklist_recepcao" },
  { href: "/produtividade", label: "Produtividade de Agendamento", icon: UserCheck, group: "OPERACOES", roles: ["ADMIN", "GESTOR", "OPERADOR"], pageKey: "produtividade" },
  { href: "/financeiro", label: "Financeiro", icon: DollarSign, group: "FINANCEIRO", roles: ["ADMIN", "GESTOR"], pageKey: "financeiro" },
  { href: "/contratos", label: "ResolveSaude", icon: CreditCard, group: "FINANCEIRO", roles: ["ADMIN", "GESTOR"], pageKey: "contratos" },
  ...
  return (
    ...
    >
      <div className="h-16 flex items-center justify-between px-4 bg-[#043563] flex-shrink-0 shadow-sm">
        <div className={cn("relative transition-all duration-300 overflow-hidden h-12", isOpen ? "w-48 opacity-100" : "w-0 opacity-0")}>
          <img
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white">
      ...
      <nav className="flex-1 px-3 space-y-6 overflow-y-auto mt-6 custom-scrollbar">
        {groups.map((group) => (
          <div key={group}>
            {isOpen && <h4 className="px-4 text-[10px] font-bold text-slate-400/70 uppercase tracking-wider mb-2">{group}</h4>}
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive ? "bg-[#17407E] text-white font-medium shadow-md" : "text-slate-300 hover:bg-white/5 hover:text-white"
                      )}
                    >
                      {isActive && <span className="absolute left-0 h-full w-1 bg-[#3FBD80] rounded-r-full top-0" />}

                      <item.icon size={20} className={cn("flex-shrink-0 transition-colors", isActive ? "text-[#3FBD80]" : "text-slate-300 group-hover:text-white")} />

                      {isOpen && <span className="ml-3 text-sm flex-1 truncate">{item.label}</span>}
      ...
      <div className="p-4 bg-[#043563] flex-shrink-0 border-t border-[#17407E]/30">
        <div className={cn("flex items-center gap-3 transition-all", !isOpen && "justify-center")}>
          ...
          {isOpen && (
            <button onClick={handleLogout} className="text-slate-400 hover:text-red-400 transition-colors p-2 rounded-md hover:bg-slate-800" title="Sair do sistema">
              <LogOut size={16} />
            </button>
          )}
        </div>
