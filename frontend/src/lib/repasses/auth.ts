...
export const requireRepassesPermission = async (action: PermissionAction) => {
  ...
  const allowed = hasPermission(permissions, 'repasses', action, role);

  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para repasses.' };
  }
