...
export type QmsPageKey =
  | 'qualidade_documentos'
  | 'qualidade_treinamentos'
  | 'qualidade_auditorias';

export const requireQmsPermission = async (pageKey: QmsPageKey, action: PermissionAction) => {
  ...
  if (!allowed) {
    return { ok: false as const, status: 403, error: 'Sem permissao para esta pagina.' };
  }
